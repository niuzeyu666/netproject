 ...
 * ============================================================ */
/* Structured event trace consumed by the congestion-control test scripts. */
#include <stdarg.h>
static pthread_mutex_t trace_lock = PTHREAD_MUTEX_INITIALIZER;
static pthread_once_t trace_once = PTHREAD_ONCE_INIT;

static const char* trace_filename(void) {
    char host[64] = {0};
    gethostname(host, sizeof(host) - 1);
    return strcmp(host, "server") == 0 ? "server.event.trace" : "client.event.trace";
}

static void trace_init(void) {
    FILE* f = fopen(trace_filename(), "w");
    if(f) {
        fputs("TJU_TCP_TRACE_BEGIN\n", f);
        fclose(f);
    }
}
...
static void trace_write(const char* fmt, ...) {
    ...
    const char* name = trace_filename();
    ...
    fprintf(f, "[%llu] ", (unsigned long long)tv.tv_sec * 1000000ULL + tv.tv_usec);
    va_list ap;
    va_start(ap, fmt);
    vfprintf(f, fmt, ap);
    va_end(ap);
...
static const char* trace_flag(uint8_t flags) {
    if(flags == (SYN_FLAG_MASK | ACK_FLAG_MASK)) return "SYN|ACK";
    if(flags == (FIN_FLAG_MASK | ACK_FLAG_MASK)) return "FIN|ACK";
    if(flags & SYN_FLAG_MASK) return "SYN";
    if(flags & FIN_FLAG_MASK) return "FIN";
    if(flags & ACK_FLAG_MASK) return "ACK";
    return "NONE";
}
...
static void emit_pkt(tju_tcp_t* s, uint8_t flags, uint32_t seq, uint32_t ack,
    ...
    free(buf);
    trace_write("[SEND] [seq:%u ack:%u flag:%s]", seq, ack, trace_flag(flags));
    trace_write("[RWND] [size:%u]", adv);
...
static void unlink_sock(tju_tcp_t* s) {
    ...
    cb->unlinked = 1;
    cb->dying = 1;
    /* refcnt includes the application/API owner.  Unlinking from the kernel
       tables must not release that owner while connect/recv/close may still
       be executing on the socket. */
    for(int i = 0; i < MAX_SOCK; i++) {
        if(listen_socks[i] == s) listen_socks[i] = NULL;
        if(established_socks[i] == s) established_socks[i] = NULL;
    }
...
static void rtt_measure(struct tcp_cb* cb, uint32_t r_ms) {
    ...
    clamp_rto(cb);
    trace_write("[RTTS] [SampleRTT:%u EstimatedRTT:%.3f DeviationRTT:%.3f TimeoutInterval:%u]",
                r_ms, cb->srtt8 / 8.0, cb->rttvar8 / 8.0, cb->rto_ms);
}
static void arm_retx(struct tcp_cb* cb, uint64_t now) {
    cb->retx_on = 1;
...
static void cong_on_ack(struct tcp_cb* cb, uint32_t newly_acked) {
    ...
    int type = cb->cwnd < cb->ssthresh ? 0 : 1;
    ...
    trace_write("[CWND] [type:%d size:%u]", type, cb->cwnd);
}
...
static void cong_on_timeout(struct tcp_cb* cb) {
    ...
    cb->cwnd = CWND_FLOOR;
    cb->fast_recovery = 0;
    trace_write("[CWND] [type:3 size:%u]", cb->cwnd);
    trace_write("[TIMEOUT] [ssthresh:%u]", cb->ssthresh);
}
...
static void cong_on_fast_rexmit(struct tcp_cb* cb) {
    ...
    cb->fast_recovery = 1;
    cb->in_recovery = 1;
    trace_write("[CWND] [type:2 size:%u]", cb->cwnd);
    trace_write("[FAST_RETRANSMIT] [ssthresh:%u recover:%u]", cb->ssthresh, cb->recover_point);
}
    ...
    if(seq_after(acknum, cb->tx_una)) {
        ...
        if(cb->fast_recovery) {
            if(seq_leq(cb->recover_point, new_una)) {
                cb->fast_recovery = 0;
                cb->cwnd = cb->ssthresh;
                trace_write("[CWND] [type:1 size:%u]", cb->cwnd);
            } else {
        ...
        if(cb->inflight_cnt > 0) {
            cb->dup_ack_cnt++;
            trace_write("[DUPACK] [ack:%u count:%d]", acknum, cb->dup_ack_cnt);
...
static tju_tcp_t* spawn_child(tju_tcp_t* listener, char* pkt) {
    ...
    cb->cwnd = (uint32_t)INIT_WDSIZE * SMSS_BYTES;
...
tju_tcp_t* tju_socket() {
    ...
    pthread_mutex_init(&cb->accept_mtx, NULL);
    pthread_cond_init(&cb->accept_cv, NULL);
    trace_write("[CWND] [type:0 size:%u]", cb->cwnd);
    return s;
}
...
int tju_recv(tju_tcp_t* s, void* buffer, int len) {
    ...
    cb->rx_read += n;
    trace_write("[DELV] [seq:%u size:%u]", cb->rx_read - n, (unsigned)n);
...
int tju_handle_packet(tju_tcp_t* s, char* pkt) {
    ...
    uint32_t seq = get_seq(pkt);
    uint32_t ack = get_ack(pkt);
    trace_write("[RECV] [seq:%u ack:%u flag:%s]", seq, ack, trace_flag(flags));
